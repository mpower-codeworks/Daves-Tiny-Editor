...
WindowCommand:
    ...
    push    OFFSET TitleText           ; lParam = filename plus marker
    push    0                          ; wParam = unused
    push    0Ch                        ; Msg    = WM_SETTEXT
    push    dword ptr [esp+10h]        ; hWnd   = callback parent HWND

    call    [__imp__SendMessageA@16]   ; refresh dirty window title
    jmp     WindowDone                 ; notification handled
...
SaveFile:
    ...
    push    OFFSET TitleText           ; lParam = clean filename title
    push    edi                        ; wParam = unused
    push    0Ch                        ; Msg    = WM_SETTEXT
    push    dword ptr [esp+18h]        ; hWnd   = callback parent HWND

    call    [__imp__SendMessageA@16]   ; display clean title after save
