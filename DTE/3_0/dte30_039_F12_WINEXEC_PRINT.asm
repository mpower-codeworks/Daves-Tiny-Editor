...
;;;;;;;;;;;;;;;;;
    mov     ebx, OFFSET __imp__SendMessageA@16
                                        ; EBX = compact SendMessage IAT slot
...
DispatchQueued:
    ...
    cmp     byte ptr [esi+8], 72h       ; VK_F3?
    je      FindNext                    ; yes = find selected text forward
    cmp     byte ptr [esi+8], 78h       ; VK_F9?
    je      SaveKey                     ; yes = trigger existing Save command
    cmp     byte ptr [esi+8], 7Bh       ; VK_F12?
    je      PrintKey                    ; yes = print saved file with Notepad
...
PrintKey:
    ...
    mov     ecx, TitleEnd               ; ECX = byte after saved path
    sub     ecx, esi                    ; ECX = path length
    rep     movsb                       ; append saved path
    mov     word ptr [edi], 22h         ; append quote plus terminating zero
...
FindNext:
    ...
    mov     eax, dword ptr [esi+4]      ; EAX = old selection end
    mov     dword ptr [esi], eax        ; search starts after selection
    or      dword ptr [esi+4], -1       ; search through end of document

FindSearch:
    push    esi                         ; lParam = FINDTEXTEX workspace
    push    1                           ; wParam = FR_DOWN
    ...
    mov     dword ptr [esi], edi        ; no match = wrap to document start
    jmp     FindSearch                  ; search again from beginning
...
PrintPrefix db 'notepad /p "',0         ; twelve copied command bytes
