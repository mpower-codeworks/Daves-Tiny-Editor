...
; 024 rewrite Save stack      - pending bytes
; 025 
; 026 
...
;;    jne     DefaultWindow              ; no = normal default behavior
    ...
    ;; unused ;; mov dword ptr [eax], 2A20h ; append space-star-zero
    mov     word ptr [eax], 2A20h      ; append space and star

;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; DirtyTitleUpdate Old  ;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; unused ;; push OFFSET TitleText
;; unused ;; push 0
;; unused ;; push 0Ch
;; unused ;; push dword ptr [esp+10h]
;; unused ;; call [__imp__SendMessageA@16]
;; unused ;; jmp WindowDone
...
;;;;;;;;;;;;;;
SaveFile:
    cmp     FileHnd, edi               ; file available for saving?
    je      WindowDone                 ; no = ignore Save command

    push    ebp                        ; preserve callback nonvolatile EBP
    push    ebx                        ; preserve HWND and hold text length
    ...
    mov     ebx, eax                   ; EBX = length across API calls
    ...
    xchg    ebp, eax                   ; EBP = allocated save buffer
    test    ebp, ebp                   ; allocation succeeded?
    jz      SaveDone                   ; no = keep dirty state

    lea     ecx, [ebx+1]               ; capacity includes terminating zero
    ...
    lea     eax, [esp+14h]             ; reuse callback wParam for byte count
    push    edi                        ; lpOverlapped = none
    ...
    pop     ebx                        ; restore main-window HWND
    pop     ebp                        ; restore callback nonvolatile EBP
    jmp     TitleUpdate                ; use shared clean-title refresh
