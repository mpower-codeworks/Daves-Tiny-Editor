...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; renamed to DTE 3.0 and renumbered ;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; 001 review for leaks, errors  - 875 bytes
;; 002 fix REDIT capacity        - 881 bytes
;; 003 direct file streaming     - test build
...
WinX86Entry:
    ...
    cmp     byte ptr [esi], 22h         ; quoted executable path?
    jne     DropExeScan                 ; no = scan for separating space

    inc     esi                         ; skip opening executable quote
...
DropSkip:
    ...
    dec     esi                         ; restore first file-path byte
    mov     cl, 20h                     ; plain file ends at space
    cmp     byte ptr [esi], 22h         ; quoted dropped path?
    jne     DropEnd                     ; no = ESI already starts path

    inc     esi                         ; skip opening file quote
...
;;;;;;;;;;;;;;
DropOpen:
    cmp     byte ptr TitleText, 0       ; file argument present?
...
WindowProc:
    ...
    sub     eax, 5                     ; WM_SIZE message?
    je      WindowSize                 ; yes = resize child control
    sub     ax, 10Ch                   ; WM_COMMAND message?
    je      WindowCommand              ; yes = test Rich Edit change
    dec     eax                        ; WM_SYSCOMMAND message?
...
;;;;;;;;;;;;;;;;;;;
WindowCommand:
    cmp     byte ptr [esp+0Fh], 3      ; HIWORD(wParam) = EN_CHANGE?
...
SaveFile:
    ...
    xor     eax, eax                    ; EAX = no stream error
...
StreamCallback:
    ...
    neg     eax                         ; CF = API returned nonzero
    sbb     eax, eax                    ; success = -1, failure = zero
    inc     eax                         ; success = zero, failure = one
    ret     10h                         ; remove four callback arguments
...
;;;;;;;;;;;
.data?
FileHnd     dd ?                ; EDITSTREAM cookie and retained file
...
TitleText   db 106h dup(?)      ; loader-zeroed path and title workspace
