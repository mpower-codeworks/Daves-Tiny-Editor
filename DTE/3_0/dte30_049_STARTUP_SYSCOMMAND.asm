...
;; 000 init and clear thread     - 372 bytes exe
...
;; 045 15-byte font face slot    - discarded
;; 046 restore 20-byte font      - 980 bytes
;; 047 window state tail         - 984 bytes
;; 048 pack state with height    - state failed
;; 049 startup syscommand tail   - 987 bytes
...
;;;;;;;;;;;;;;;;;;;;;;;
;; patchable setup tail ;;
;;;;;;;;;;;;;;;;;;;;;;;
WINDOW_CMD_ADDR  EQU 4003E4h            ; file offset 996 startup command
                                        ; F120h = SC_RESTORE / windowed
                                        ; F030h = SC_MAXIMIZE
FONT_FACE_ADDR   EQU 4003E8h            ; file offset 1000 in mapped image
FONT_HEIGHT_ADDR EQU 4003FCh            ; file offset 1020 signed height
...
DropReady:
    ...
    push    258h                        ; nHeight      = 561 px
    push    320h                        ; nWidth       = 784 px
    push    edi                         ; Y            = zero
    push    edi                         ; X            = zero
...
FontZeros:
    ...
    push    dword ptr [eax+14h]         ; nHeight follows 20-byte face slot
...
ResizeProc:
    ...
    push    dword ptr [WINDOW_CMD_ADDR] ; wParam = RESTORE or MAXIMIZE
    push    112h                        ; Msg    = WM_SYSCOMMAND
    push    dword ptr [esi+7Fh]         ; hWnd   = retained main window
    call    dword ptr [ebx]             ; route through installed WindowProc
