...
;; 045 15-byte font face slot    - 980 bytes - discarded
;; 046 restore 20-byte font      - 980 bytes
;; 047 window state tail         - 984 bytes
;; 048 pack state with height    - build to measure
...
FONT_HEIGHT_ADDR EQU 4003FCh             ; file offset 1020 packed DWORD
                                         ; low word = signed font height
                                         ; bit 24   = WS_MAXIMIZE startup flag
...
DropReady:
    ...
    push    edi                         ; Y            = zero
    push    edi                         ; X            = zero
    mov     eax, dword ptr [FONT_HEIGHT_ADDR]
                                         ; bit 24 carries configured startup state
    and     eax, 01000000h               ; keep only WS_MAXIMIZE flag
    or      eax, 10CF0000h               ; base = visible overlapped window
    push    eax                          ; style = configured startup state
...
FontZeros:
    ...
    movsx   eax, word ptr [eax+14h]      ; low word = signed configured height
    push    eax                          ; nHeight = sign-extended font height
