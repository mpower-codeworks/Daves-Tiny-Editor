...
; run BUILD from command line for 890 byte version
; this version must be compiled in a directory that
; has been excluded from Windows Defender Antivirus
; or Windows will flag a false positive
; for example c:\ASSEM_TEST

; do not disable antivirus protection
; you can use a single test folder exclusion
; (this can be configured via PowerShell)

; you can use the ALT BUILD version for regular
; usage (about 4.5k size) and there won't be any
; false-positive warnings

; BACKUPS folder provides a history of building
; up DTE from the original tiny.asm
...
;include gdi32.inc         ; Removed because no GDI used for editor
                           ; you will need it if you want to change
                           ; fonts though...
...
EXTERN _imp__RegisterClassA@4      :PTR
...
ParseStartupFile proc NEAR
    ...
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ;; if called from cmd line without ;;
    ;; a startup file,  just close dte ;;
    ;;          saves 5 bytes          ;;
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    NoArg:
